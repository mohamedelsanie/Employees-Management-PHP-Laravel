                    ...
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">
                                {{ __('admin/admin.index.title') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    @if(AdminCan('admin-create'))
                    <a href="{{ route('admin.admins.create') }}" class="btn btn-primary btn-sm scroll-click"><i class="fa fa-plus"></i> {{ __('admin/admin.index.create') }}</a>
                    @endif
                    @if(AdminCan('admin-forcedelete'))
                    <a href="{{ route('admin.admins.archive') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/admin.index.archive') }}</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="row">
                    <div class="col-md-6 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/admin.index.title') }}</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <form name="search_form" method="post" action="{{route('admin.admins.search')}}" >
                            @csrf
                ...
                <form name="destroy_all" method="post" action="{{route('admin.admins.destroy_all')}}" >
                    @csrf
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="table-actions">
                                            @if(AdminCan('admin-edit'))
                                                <a href="{{ route('admin.admins.edit',$admin->id) }}" data-color="#265ed7" style="color: rgb(38, 94, 215);"><i class="icon-copy dw dw-edit2"></i></a>
                                            @endif
                                            @if(AdminCan('admin-delete'))
                                                    <a x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-deletion_{{ $admin->id }}')" data-color="#e95959" style="color: rgb(233, 89, 89);  cursor: pointer;"><i class="icon-copy dw dw-delete-3"></i></a>
                                                    <x-modal name="confirm-deletion_{{ $admin->id }}" focusable>
                                                        <div class="p-6">
                                                            <h2 class="text-lg font-medium text-gray-900">
                                                                {{ __('admin/common.messages.delete_title',['name' => $admin->name]) }}
                                                            </h2>
...
{{--                                                <a href="{{ route('admin.admins.delete',$admin->id) }}" data-color="#e95959" style="color: rgb(233, 89, 89);"><i class="icon-copy dw dw-delete-3"></i></a>--}}
                        ...
                        @else
                            <tr>
                                <td colspan="4">{{ __('admin/admin.index.notfound') }}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
    ...
    @section('page_title'){{ __('admin/admin.index.title_tag') }}@endsection
</x-admin-layout>
