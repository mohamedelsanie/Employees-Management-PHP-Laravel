<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('admin.categories.index') }}">{{ __('admin/categories.index.title') }}</a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">
                                {{ __('admin/categories.archive.title') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    <a href="{{ route('admin.categories.index') }}" class="btn btn-primary btn-sm scroll-click">
                        {{ __('admin/categories.archive.all') }}</a>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/categories.archive.title') }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(function() {
            var table = $('#categories-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ Route('admin.categories.trashed') }}",
                columns: [{
                    data: 'id',
                    name: 'id',
                    orderable: false,
    ...
    @endpush
    @section('page_title'){{ __('admin/categories.archive.title_tag') }}@endsection
</x-admin-layout>
